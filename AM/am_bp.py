...
def submit_signup(action_id):
    ...
    emailer.queue_email("AM/confirm_participation.html", 
                        subject="Vahvista osallistumisesi!",
                        recipients=[email], context={"name": name, "action_id": action_id, "action": Action.from_dict(actions_collection.find_one({'_id': ObjectId(action_id)})), "confirmation_link": url_for("am.confirm_participation", hash=hash, _external=True)})
...
def confirm_participation(hash):
    ...
    if not signup["confirmed"]:
        ...
        action = Action.from_dict(actions_collection.find_one({'_id': ObjectId(signup["action_id"])}))
        
        DB["action_signups"].update_one({"_id": signup["_id"]}, {"$set": {"confirmed": True}})
        
        emailer.queue_email(
            "AM/confirmation_email.html",
            subject="Osallistumisesi vahvistettu!",
            recipients=[signup["email"]],
            context={"name": signup["name"], "action": action, "cancel_link": url_for("am.cancel_participation", hash=hash, _external=True)},
        )

        return render_template('AM/confirm_participation.html', signup=signup, action=action)
...
def cancel_participation(hash):
    ...
    DB["action_signups"].update_one({"_id": signup["_id"]}, {"$set": {"cancellation_started": True, "last_updated": datetime.datetime.now()}})
    

    return render_template('AM/cancel_participation.html', cancel_confirm_link=url_for("am.confirm_cancel", hash=hash))
...
def confirm_cancel(hash):
    ...
    DB["action_signups"].update_one({"_id": signup["_id"]}, {"$set": {"cancelled": True, "last_updated": datetime.datetime.now()}})
    
    emailer.queue_email(
        "AM/cancellation_email.html",
        subject="Osallistumisesi peruutettu!",
        recipients=[signup["email"]],
        context={"name": signup["name"], "action": Action.from_dict(actions_collection.find_one({'_id': ObjectId(signup["action_id"])}))},
    )
