...
### Fixed
* The admin's combined 24-hour view analytics now uses the same responsive section card, filter, accessible data table, empty/error states, and theme-aware chart tokens as the other analytics views. It keeps server-rendered values available without JavaScript, refreshes safely without overlapping requests, and updates chart colors immediately when the admin theme changes.
