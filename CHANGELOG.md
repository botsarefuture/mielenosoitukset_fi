...
### Changed
* The hallintaloki and global-admin technical audit log now use the shared admin tabs, filters, result summaries, data views, disclosures, empty states, and pagination. Hallintaloki entries are rendered safely on the server instead of being interpolated into `innerHTML`; both logs use deterministic server-side pagination with every filter preserved, and the technical log's former arbitrary 10–1000 row cap is removed.
