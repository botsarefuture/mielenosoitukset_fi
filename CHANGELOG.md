...
* The public "Suggest a change" form now supports correcting an event's organizers. A new "Järjestäjät" pill-editor section lets visitors add, edit, and remove organizer entries (name plus optional email/website in parentheses, e.g. `Yhdistys (info@example.fi, https://yhdistys.fi)`), the submitted organizers are normalized into the same list-of-dicts shape the demonstration document uses, unchanged organizer lists are not recorded, and the admin suggestion review view renders organizer diffs as readable "Name (contact)" lines so the existing field-level approval flow can apply them as-is.
* **Built-in first-party analytics** — pageviews are now recorded server-side in the Flask request lifecycle, so basic analytics work without cookies, JavaScript, localStorage, or any third-party service (even cached pages are counted). Nothing visitor-identifying is stored: every tracked request only bumps an atomic aggregate counter in a new `site_analytics` collection (one document per Helsinki-local date/hour × page type × resource × language × device × referrer), created with indexes by an automatic migration. Static assets, admin/account surfaces, APIs, health checks, analytics endpoints, non-HTML responses, error responses and obvious bots are never counted. A new **Palvelun analytiikka** admin dashboard (sidebar, `VIEW_ANALYTICS` permission) shows pageviews today/yesterday/this week/this month with comparisons to previous periods, a daily traffic chart (7/14/30/90-day ranges), top pages, top demonstrations, and language/device/referrer breakdowns. Each demonstration's command center links to a per-demonstration analytics page with totals, daily views, and language/device/referrer splits; the classifier also unifies a demo's ObjectId, slug and running-number URLs into the same counters.
* **Analytics events** — deliberate user actions are now recorded into the same aggregate counters: demonstrations submitted, reminder subscriptions, organization/recurring follows and unfollows, contact messages, campaign volunteer signups, language changes, and demonstration-list searches (first page of a search only, so pagination doesn't inflate counts; city/tag filtering is intentionally not recorded since city views are already counted as pageviews). A tiny allowlisted browser beacon (`/api/analytics/event` + `static/js/site_analytics.js`) reports only two things that requests cannot reveal: map interactions (once per page load) and clicks on external links (target hostname only). The beacon rejects unknown events and cross-origin submissions, event resource values are length-capped, bots are skipped everywhere, and the site analytics dashboard gained "Tapahtumat" (event totals) and "Yleisimmät haut" (top search terms) cards.
* New `SITE_ANALYTICS_ENABLED` config flag (default on) lets a deployment stop recording pageviews without touching dashboards, and `MATOMO_ENABLED` (default on) controls the legacy client-side Matomo tracker in `base.html`, the admin stats page, and the campaign confirmation page.
...
* **Facebook import now imports the event's cover photo** (`imageUrl` from the Apify actor) and uploads it to S3 as the demo's main image; the image previews in the form and is carried forward to submission if the user doesn't upload their own file.
...
### Changed
* Public and admin interfaces now share one neutral `product-tokens.css` foundation for primary/accent colors, actions, surfaces, text, borders, feedback states, canvas, and elevation. Public components consume `--product-*` tokens directly, while the stable `--admin-workspace-*` API aliases those same values inside the admin workspace; this removes admin-scoped token names from public CSS without changing either interface's purpose-specific layout.
* The public notification popover now uses the shared product tokens and component states for its light/dark surfaces, focus treatment, responsive layout, loading, empty, error, unread, and disabled states. Notification content is rendered with safe DOM nodes instead of interpolated HTML, internal links are constrained to same-origin paths, and keyboard users can close the popover with Escape and return focus to its trigger.
...
* The public demonstration list, city, and tag pages now use a shared hero macro, keeping the standard site hero and compact city/tag page banners consistent while preserving their localized content and cover-image behavior.
* The public list, city, and tag views now share one listing controller and grid: every page renders `section.main-content` → `section.section-card` → `#demos-grid.demos-grid` with the shared `load_more_button()` + `UserPagination` button pagination and `demo-card-render.js` (`/api/demonstrations`), so the city page is visually and behaviourally identical to the main and tag listings (3 cols ≥1200px, 2 cols 768–1199px, 1 col ≤767px via `user-workspace.css`). The city listing is now a client-side shell (`/city/<city>` passes only `city_name`/`current_locale`; server-side pagination removed) with a locked `extraParams.city` and `display_date_start` → `date_start` ISO conversion — replacing its legacy `container-grid` (4-per-row `auto-fit minmax(300px,1fr)`), `view-toggle`, `demo-table`, `scroll_sentinel` + IntersectionObserver, and `listing_end_message` infinite scroll.
...
### Fixed
* The public API-token manager now requires authentication and uses shared product-token page headers, data cards, check rows, empty/loading states, modals, focus handling, and responsive actions in light and dark themes. Token/API data is rendered with DOM nodes instead of `innerHTML`, native `confirm()` is replaced by an accessible confirmation modal, actions expose busy states, and the former misleading “view” action is labelled as creating a new equivalent token. Token creation and revocation return localized safe errors; malformed, missing, foreign, and already-revoked IDs return controlled 400/404 responses instead of 500 errors.
* Registration now enforces the same password contract in the browser and on the server. Empty username/email fragments no longer make every password look as if it contains personal information, the backend evaluates the boolean returned by `is_strong_password` instead of treating its `(result, message)` tuple as always truthy, and both layers share the accepted special-character set. The form also uses localized safe DOM status/error rendering, shared product-token validation states, CSS-based strength/progress styles, and the site's existing font/icon load instead of page-owned inline CSS and duplicate CDNs.
* The registration next-steps page now keeps the submitted email address across its redirect, so the resend control renders instead of JavaScript calling `addEventListener` on a missing form. Its progress steps use shared light/dark product components, duplicate third-party font/icon loads and inline CSS are removed, resend and verification controls have explicit loading/current-step states, user data is serialized safely into JavaScript, and resend failures return localized messages without leaking backend exception details.
* The account profile editor now uses the shared public form, field, help-text, preview, focus, disabled, and action components in both themes. The profile image is explicitly optional and leaving it empty preserves the current picture; saving exposes a proper busy state, feedback is rendered with safe DOM nodes, and the profile API now requires authentication, returns localized messages, and no longer leaks image-decoding exception details. The obsolete second profile editor and its page-specific stylesheet were removed so account settings is the single canonical editor.
...
* Demonstration audit history and the cross-demonstration audit timeline now share the canonical admin activity-stream, filter, result-summary, empty-state, and pagination components. Both use deterministic server-side pagination; the timeline applies city/organization/global `VIEW_DEMO` scope before calculating totals or rows, accepts both BSON and string identifiers used by existing demonstrations, and sends view-only administrators back to the authorized command center, so restricted administrators neither receive audit events outside their scope nor hit an edit-only dead end.
...
* The admin's combined 24-hour view analytics now uses the same responsive section card, filter, accessible data table, empty/error states, and theme-aware chart tokens as the other analytics views. Its initial server-rendered values use the same Helsinki-local time window as the refresh API, remain available without JavaScript, refresh safely without overlapping requests, and update chart colors immediately when the admin theme changes.
* Demonstration edit history and version comparison now use the canonical admin hero, breadcrumb/back action, data-view, pagination, modal, and theme contracts. History pagination is deterministic and server-side, rollback requires an explicit shared confirmation dialog, and every line rendered into a trusted diff is escaped so stored demonstration content cannot inject HTML into an administrator's browser.
* The site analytics dashboard no longer splits each row's label and count across the card (previously the count could end up far right, requiring horizontal scrolling to read): breakdown cards (languages, devices, referrers, events, top searches) now use compact label-left/count-right rows that always fit, and page types without a resource (submit form, calendar, front page, "Mielenosoitukset tänään") get proper Finnish labels in the top-pages list instead of raw identifiers. A note on the dashboard now documents what is deliberately not counted (admin panel, account area, APIs, static files, bots) and that individual visitors are never identified or stored.
* Demonstration detail pages served from the app cache no longer go completely untracked by the legacy demo-view logging: the built-in server-side analytics counts every rendered response regardless of cache state (the old `log_demo_view` path remains for the beacon endpoint).
...
* Admin summary-card icons now use an explicit cross-browser flex and glyph-centering contract, so Font Awesome symbols remain visually centered inside their colored tiles in Firefox as well as Chromium at desktop, mobile, and zoomed layouts.
...
* The admin workspace CSS cache-buster contract test no longer pins a hardcoded date: the shared admin shell version (`css/admin/workspace.css?v=<date>-admin-ui-N`) is asserted with a date-agnostic pattern, so bumping the workspace CSS version no longer breaks CI.
...
* Passkey sign-in bypasses the TOTP step (passkeys are stronger multi-factor credentials) but still requires a confirmed email address.
* Recurring-demonstration create and edit forms now show a live, permission-protected preview of the next occurrence dates from the same calculation used by the background worker, including the selected monthly day, end date, and excluded break dates, before any child demonstrations are generated. Long consecutive breaks are scanned past safely, and obsolete preview responses cannot overwrite newer form state.
...
* Added a new background job `process_email_queue` (runs every 2 minutes) that drains the `email_queue` collection directly, ensuring queued emails (admin ticket replies, auto-replies, etc.) are sent even when the originating `EmailSender` process dies. This fixes a bug where emails were stuck in the queue after process restarts or when scheduler scripts exited before their daemon threads could process jobs.
...
### Changed
* Removed the retired standalone admin media workspace, including its unprotected upload/list routes, obsolete S3 templates, browser upload script, blueprint registration, test stubs, and page-specific workspace CSS. Demonstration, recurring-demonstration, organization, and profile image workflows continue to use their existing scoped upload fields and shared S3 helpers.
* Developer-access requests, application-scope requests, and per-user developer apps now use the shared admin tabs, filters, result summaries, semantic statuses, data views, empty states, safe toast feedback, and server-side pagination. Stable compound ordering and preserved URL state replace unbounded Bootstrap tables, while existing `EDIT_USER` authorization and decision endpoints remain unchanged.
* The demonstration merge editor now uses the canonical admin form sections, comparison cards, facts, check/radio rows, disclosures, help text, and sticky actions. Guided and manual modes keep the existing field names and merge behavior, while the manual controls use semantic `hidden` state instead of inline display mutations; the obsolete merge-only CSS component layer has been removed.
* The UI-translation editor now uses the same responsive admin detail layout, section cards, fields, help text, sticky actions, semantic statuses, permission-aware empty state, and review controls as other admin forms. Source context, proposal metadata, GitHub-sync state, translator submission, and reviewer decisions remain visually coherent in both themes without Bootstrap card/grid styling.
* The global-admin one-time-link registry now uses the shared admin filter, result-summary, data-view, disclosure, confirmation-modal, empty-state, and server-side pagination contracts. Counts stay truthful under filters, stable ordering prevents skipped or repeated links between pages, and revocation no longer relies on an inline browser confirmation handler.
* Background-job details now use the same section cards, semantic status badges, result summaries, empty states, filters, disclosures, and server-side pagination as the main job collection. Run filters and page size remain in the URL, and the responsive light/dark layout no longer depends on Bootstrap card or list-group palettes.
* The final shared admin accessibility contract now covers keyboard-visible focus, reduced motion, long localized hero titles, sticky action geometry, internally scrollable data tables, and the 720 CSS-pixel layout produced by 200% zoom on a 1440px desktop. Shared mobile navigation, table-row, hero-action, and save-bar transitions are disabled when reduced motion is requested.
* The shared admin shell's sidebar, task card, theme switch, toast layer, modal stacking, and footer styles now live in the workspace component stylesheet and use its semantic light/dark tokens. The offcanvas sidebar keeps its compact width throughout Bootstrap's full mobile breakpoint, the base template no longer contains inline CSS or static `style=` attributes, and a contract test prevents that shell-level styling debt from returning.
* Demonstration suggestions now use the shared, responsive admin collection and review components with permission-scoped totals, deterministic server-side pagination, clear multi-select state, and a common confirmation modal; city and organization administrators no longer receive global suggestion rows or counts. Status filtering, counts, and pagination run as database queries against the accessible demonstration scope (mirroring the demo dashboard) instead of loading the whole suggestion collection at once, and clearing the status filter keeps the selected page size.
* Admin V2 pages now use the canonical page, panel, toolbar, form-section, and data-view primitives directly; the retired dashboard/page/table compatibility classes and their dead CSS selectors have been removed and guarded by a contract test.
* The admin shell no longer loads the obsolete `variables.css` and `admin_v2.css` theme layers. Their remaining form markers, actions, footer version, table action cells, disabled hero actions, and demo-editor colors now use the shared semantic workspace contract; editor focus rings reliably override resting field styles; dead demo-checkbox script requests no longer produce a 404; and a ratchet rejects legacy admin tokens outside the separately documented public toolbar modal.
* Removed the unused pre-workspace admin activity, case, dashboard, recurring-form, checkbox, and sidebar stylesheets together with an unreachable recurring-demo form and its invalid legacy table macro. A contract check now prevents these obsolete page-specific UI layers from returning.
* Admin statistics now render their permission-scoped demonstration analytics as a shared server-side data view with stable pagination, a canonical empty state, and preserved period/page-size filters. Scoped administrators no longer receive global user, organization, Matomo, or unrelated demonstration totals, and the obsolete statistics-specific palette, table, and pagination layer has been removed.
* The hallintaloki and global-admin technical audit log now use the shared admin tabs, filters, result summaries, data views, disclosures, empty states, and pagination. Hallintaloki entries are rendered safely on the server instead of being interpolated into `innerHTML`; both logs use deterministic, compound-index-backed server-side pagination with every filter preserved, and the technical log's former arbitrary 10–1000 row cap is removed.
* Governance overview, superuser-clearance management, and clearance audit now use the canonical admin summary, tabs, filters, result headers, data tables, empty states, status badges, and pagination. Clearance and audit searches are server-rendered with deterministic ordering and preserved URL state, and the obsolete governance-specific Bootstrap palette has been removed.
* Demonstration translations, UI translations, and GitHub translation-sync queues now use the same admin filter, result-summary, data-table, empty-state, bulk-selection, and server-rendered pagination components as other admin collections. Their former 100/200/300-row caps are removed, filters persist across pages, ordering is deterministic, and the shared page-size control now has one workspace-level behavior instead of template-specific copies.
...
### Fixed
* The shared admin sidebar now identifies the authenticated user's real global, city, organization, translator, or restricted scope instead of telling every admin-shell user that they are in superuser mode. "Globaali ylläpito" is reserved for accounts with full-permission bypass (`global_admin` or a truly global role); an `admin`-role account with only selected permissions is shown with the reduced scope instead. The scope lookup only runs for admin-shell renders, so public pages no longer trigger extra permission-scope database queries.
* The recurring-demonstration collection now applies permission scope, search, approval filtering, stable server-side pagination, truthful result counts, and the same shared page-size/footer contract as the other admin collections instead of filtering only the current browser table. Pagination counts and the page rows both ignore records the dashboard cannot parse (malformed `repeat_schedule` / `created_until`), so advertised totals never exceed the rendered rows. Deleting a series from the dashboard now actually works for users holding only `DELETE_RECURRING_DEMO`: the delete endpoint accepts the modal's JSON confirmation and answers with a proper JSON status, instead of bouncing back to a redirect the modal cannot read.
* **Email delivery is now reliable and never silently loses queued messages** (`EmailSender` + `process_email_queue`). Previously the queue workers did `find_one_and_delete` *before* sending, so any SMTP failure permanently destroyed the job while the admin UI reported success — the exact cause of support-ticket replies (e.g. ticket #100156) never reaching recipients. Jobs are now claimed atomically (`status: pending → in_flight`), only deleted after a successful send, and marked `failed` with a stored `last_error` so the 2-minute `process_email_queue` background job retries them (capped attempts with a retry cooldown). Stale `in_flight` claims from crashed processes are re-queued automatically. SMTP connections also get a 30 s socket timeout so flaky connections fail fast and are retried instead of hanging.
* **Admin ticket replies no longer overwrite the outgoing-message record**: `reply_to_ticket` recorded case history/actions after `queue_admin_reply`, and the full-document `_touch()` save then wiped `suggestion.messages` (observed as an empty follow-up list on ticket #100156). The audit trail and action log are now written with targeted `$push` updates *before* the reply is queued, and each outbound message records its real delivery status (`queued` → `sent`/`failed` with error + timestamp), which is shown as a badge in the case follow-up list. The success flash no longer claims immediate delivery.
* The demo dashboard's row actions dropdown for unaccepted demonstrations again includes a "Hylkää" (reject) action. It posts to the existing reject API with the same `ACCEPT_DEMO` permission check as "Hyväksy", confirms before acting, and removes the rejected row from the list.
* The public demonstration page's "Ilmoita virheestä" dialog buttons no longer inherit the page-level `.btn` styles (shimmer `::before` overlay and `border: none`): modal buttons now render clean and the "Haluan vain ilmoittaa virheen" outline button keeps its visible 1px border on both mobile and desktop.
* The public demonstration page's report and choice dialogs now cap their height to the viewport so neither modal clips above/below the screen on mobile or overflows horizontally; on small screens the entire dialog scrolls as one swipeable region (swiping from the header/hint/cards scrolls it too), the two-choice cards are more compact, and the report dialog buttons sit side by side. Client-drag scrolling previously only worked from the small body area.
...
* Shared admin review regressions are resolved: disabled hero actions keep canonical styling, small warning labels use a contrast-safe semantic foreground, the demonstration table stays contained at intermediate widths, reduced-motion deletion completes immediately, and light-theme modal close controls remain visible.
* Case summary cards and case-detail section headings now use the shared component wrappers, restoring the standard label/value flow, compact heading typography, and responsive overflow behavior.
* Campaign volunteer administration now initializes its Bootstrap edit modal only after the document and Bootstrap bundle are ready, so an early `bootstrap is not defined` error can no longer disable filtering, pagination, exports, or row actions.
* Approving a demonstration from the collection now updates the approval cell itself with the shared success state instead of accidentally overwriting the recurring-status column and leaving stale “Ei” text behind.
...
* Campaign volunteer management now uses the shared admin summary, section, filter, data-view, status, pagination, modal, checkbox, and button contracts instead of a 597-line template-local design system; its client-side filters and pagination now keep the complete filtered result set stable across page changes.
...
* Standard public pages now use one cohesive, compact and centered hero component with consistent width, restrained height, blue gradients, typography, labels, calls to action, spacing, and explicit light/dark theme styling; campaign and guide pages retain their intentional visual identities.
...
### Added
* Added a CSS integrity test that scans every shipped `static/css` stylesheet for unbalanced braces (ignoring string and comment content) and fails on any unclosed block, so an unclosed `@media`/rule brace — which hides every later rule from normal desktop viewports — cannot ship silently again.
