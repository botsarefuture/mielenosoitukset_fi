...
class RepeatSchedule:
    ...
    def __str__(self) -> str:
        ...
        if self.frequency in frequency_map:
            return f"every {self.interval} {frequency_map[self.frequency]}{'s' if self.interval > 1 else ''}{weekday_str}" if self.interval > 1 else frequency_map[self.frequency] + weekday_str
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'RepeatSchedule':
        return cls(frequency=data["frequency"], interval=data["interval"], weekday=data.get("weekday"))

class RecurringDemonstration:
    def __init__(self, title: str, start_time: datetime, end_time: datetime, topic: str,
                 facebook: str, city: str, address: str, demo_type: str, route: str,
                 organizers: Optional[List[str]] = None, linked_organizations: Optional[List[str]] = None,
                 repeat_schedule: Optional[RepeatSchedule] = None, created_until: Optional[datetime] = None, 
        ...
        while demo_date <= end_date:
            ...
            if self.repeat_schedule.frequency == "daily":
                ...
                if self.repeat_schedule.weekday:
                    days_ahead = (self.repeat_schedule.weekday.weekday() - demo_date.weekday() + 7) % 7
    ...
    def update_demo(self, title: Optional[str] = None, start_time: Optional[datetime] = None,
                    end_time: Optional[datetime] = None, topic: Optional[str] = None,
                    facebook: Optional[str] = None, city: Optional[str] = None,
                    address: Optional[str] = None, demo_type: Optional[str] = None,
                    route: Optional[str] = None, organizers: Optional[List[str]] = None,
                    linked_organizations: Optional[List[str]] = None) -> None:
        ...
        if facebook: self.facebook = facebook
        if city: self.city = city
        if address: self.address = address
        if demo_type: self.type = demo_type
        if route: self.route = route
        if organizers is not None: self.organizers = organizers or []
        if linked_organizations is not None: self.linked_organizations = linked_organizations or []
